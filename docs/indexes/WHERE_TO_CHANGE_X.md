# Where To Change X

Use this file as a fast change guide before editing.

## Public front door and repo story

- `../../README.md`
- `../../REPO_INDEX.md`
- `../INDEX.md`
- `../specs/PUBLIC_THESIS_OBJECT.md`

## Claude Code compatibility

- `../../CLAUDE.md`
- `../../.claude/settings.json`
- `../../.claude/hooks/emit-governed-event.mjs`

## VS Code extension metadata and packaging

- `../../extension/package.json`
- `../../extension/README.md`
- `../../extension/.vscode/launch.json`
- `../../extension/tsconfig.json`

## Extension runtime behavior

- `../../extension/src/extension.ts`
- `../../extension/src/hookEvents.ts`
- `../../extension/src/artifactStore.ts`
- `../../extension/src/panelViews.ts`
- `../../extension/src/panelHtml.ts`
- `../../extension/src/types.ts`

## Proof and release lane

- `../../proof/PROOF_MAP.md`
- `../../proof/RELEASE_CHECKLIST.md`
- `../../proof/CAPTURE_RUNBOOK.md`
- `../../docs/RELEASE_NOTES_0.2.0.md`
- `../../examples/session.md`
- `../../examples/session.json`

## Skills and bootstrap substrate

- `../../skills/hold-doctrine-SKILL.md`
- `../../skills/fire-team-session-SKILL.md`
- `../../skills/governed-repo-init-SKILL.md`
- `../../resources/RESOURCE_PACK_NOTE.md`
- `../../resources/WHY_THIS_KIT.md`
- `../../resources/REPO_SKELETON.md`
- `../../resources/templates/`

## Governance and contracts

- `../../TEAM_CHARTER.md`
- `../../AI_EXECUTION_DOCTRINE.md`
- `../../CONTRIBUTING.md`
- `../../MIGRATIONS.md`
